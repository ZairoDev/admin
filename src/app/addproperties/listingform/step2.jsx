...
const Step2 = ({ nextStep, prevStep }) => {
  ...
  const onSubmit = (data) => {
    Object.entries(data).forEach(([key, value]) => {
      updateLocalStorage("page2", key, value);
    });
    nextStep();
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="mt-4">
          ...
          <Controller
            name="placeLocation"
            control={control}
            rules={{ required: "Place location is required." }}
            render={({ field }) => <Input {...field} />}
          />
          {errors.placeLocation && (
            <p className="text-red-500 text-xs ml-1 mt-1">
              {errors.placeLocation.message}
            </p>
          )}
        </div>
            ...
            <Controller
              ...
              rules={{
                ...
                pattern: {
                  value: /^[0-9]+$/,
                  message:
                    "Postal code must contain only numbers without spaces.",
                },
              }}
              render={({ field }) => (
                <Input
        ...
        {/* For Future use that gives you access that this property pinned on the map */}
        <div className="mt-4">
          <p className="ml-1">Coordinates</p>
          <div className="flex ml-1 items-center gap-x-20">
            <p> Latitude{0}</p>
